...
# Skip all tests in this file on platforms without matrix dependencies (e.g., macOS)
try:
    ...
    from nanobot.channels.matrix import matrix as matrix_module
    from nanobot.config.schema import MatrixConfig
except ImportError:
    pytestmark = pytest.mark.skip(reason="Matrix dependencies not installed on this platform")
    # Define a dummy so the module can at least be imported
    matrix_module = None  # type: ignore
    OutboundMessage = None  # type: ignore
    MessageBus = None  # type: ignore
    MatrixChannel = None  # type: ignore
    MatrixConfig = None  # type: ignore
    MATRIX_HTML_FORMAT = ""
    TYPING_NOTICE_TIMEOUT_MS = 0
else:
